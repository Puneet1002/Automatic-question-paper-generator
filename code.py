import sqlite3from tkinter
import *import webbrowserimport
smtplibimport osfrom email.mime.multipart
import
MIMEMultipartfrom email.mime.text import MIMETextfrom
email.mime.base import MIMEBasefrom email import encodersimport
fpdfimport randompdf = fpdf.FPDF(format='letter')from PIL import
ImageTk, Imageconn=sqlite3.connect('paper.db')c=conn.cursor()
# to view all the table deatials in the
databasec.execute("SELECT name FROM sqlite_master where
type='table';")print(c.fetchall())
#c.execute("""CREATE TABLE EASY6(ID INT PRIMARY KEY NOT NULL, QS
CHAR(1000) NOT NULL)""")#c.execute("INSERT INTO testpaper VALUES
('Difference between A* and AO*',5,37)")#c.execute("SELECT *
FROM testpaper")#print(c.fetchall())#conn.commit()#conn.close()
def pdfgen(list1,list2,list3):
pdf.add_page()
pdf.set_font("Arial", size=15)
pdf.cell(200,15,"Question paper NCU 2018", ln=1, align="C")
pdf.set_font("Arial",'i', size=14)
pdf.cell(200,15,"Generated using an automated paper
generation system", ln=1, align="C")
...
canvas=Canvas(main,width=470,height=80,relief='raised',borderwid
th=3)
canvas.grid(row=0,column=1,padx=50,pady=20)
canvas.create_text(250,50,fill="blue",font="Times 13 italic
bold",
